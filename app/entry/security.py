from __future__ import annotations

import hashlib
import re

from app.entry.models import SecurityFlags


PHONE_RE = re.compile(r"(?<!\d)1[3-9]\d{9}(?!\d)")
EMAIL_RE = re.compile(r"[\w.+-]+@[\w-]+(?:\.[\w-]+)+")
ID_CARD_RE = re.compile(r"(?<!\d)\d{17}[\dXx](?!\d)")
BANK_CARD_RE = re.compile(r"(?<!\d)\d{13,19}(?!\d)")
TOKEN_RE = re.compile(r"\b(?:sk|ak|pk|token)[-_][A-Za-z0-9_-]{12,}\b", re.I)
PROMPT_INJECTION_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("ignore_previous_instructions", re.compile(r"ignore\s+(?:all\s+)?previous\s+instructions?", re.I)),
    ("ignore_above_instructions", re.compile(r"忽略(?:以上|前面|之前)指令")),
    ("leak_system_prompt", re.compile(r"(?:泄露|输出|打印).{0,12}(?:system prompt|系统提示词|系统指令)", re.I)),
    ("print_hidden_rules", re.compile(r"(?:打印|输出|展示|print|show).{0,12}(?:隐藏规则|隐藏指令|内部规则|hidden rules?|hidden instructions?)", re.I)),
    ("output_developer_message", re.compile(r"(?:输出|打印|泄露).{0,12}(?:developer message|开发者消息)", re.I)),
    ("bypass_safety", re.compile(r"(?:绕过|无视|突破).{0,12}(?:安全限制|安全规则|限制)")),
)
MALICIOUS_INPUT_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("script_tag", re.compile(r"<\s*script\b", re.I)),
    ("shell_delete", re.compile(r"\brm\s+-rf\b|\bdel\s+/[sq]\b", re.I)),
)


def build_security_flags(text: str) -> SecurityFlags:
    normalized = str(text or "").strip()
    redacted = redact_text(normalized)
    has_pii = redacted != normalized
    reasons: list[str] = []
    injection_reasons = _matched_reasons(normalized, PROMPT_INJECTION_PATTERNS)
    malicious_reasons = _matched_reasons(normalized, MALICIOUS_INPUT_PATTERNS)

    if has_pii:
        reasons.append("pii_redacted")
    reasons.extend(injection_reasons)
    reasons.extend(malicious_reasons)

    return SecurityFlags(
        has_pii=has_pii,
        prompt_injection_risk=bool(injection_reasons),
        malicious_input_risk=bool(malicious_reasons),
        redacted_text=redacted if has_pii else None,
        text_hash=hash_text(normalized),
        reasons=reasons,
    )


def redact_text(text: str) -> str:
    value = str(text or "")

    value = PHONE_RE.sub(lambda m: m.group(0)[:3] + "****" + m.group(0)[-4:], value)
    value = EMAIL_RE.sub(_mask_email, value)
    value = ID_CARD_RE.sub(lambda m: m.group(0)[:4] + "**********" + m.group(0)[-4:], value)
    value = BANK_CARD_RE.sub(lambda m: m.group(0)[:4] + " **** **** " + m.group(0)[-4:], value)
    value = TOKEN_RE.sub(lambda m: m.group(0)[:4] + "***", value)

    return value


def hash_text(text: str) -> str:
    return hashlib.sha256(str(text or "").encode("utf-8")).hexdigest()


def _mask_email(match: re.Match[str]) -> str:
    email = match.group(0)
    name, _, domain = email.partition("@")
    if not name:
        return "***@" + domain
    return name[:1] + "***@" + domain


def _matched_reasons(text: str, patterns: tuple[tuple[str, re.Pattern[str]], ...]) -> list[str]:
    return [reason for reason, pattern in patterns if pattern.search(text)]
